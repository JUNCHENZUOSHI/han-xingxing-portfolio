...
export const cases = {
  ...
  'sidekick-agent': {
    ...
    /* ── Narrative Hook ── */
    narrativeHook:
      '跨境商家执行 BFCM 大促这类复合经营任务，需要串联多个工具与第三方应用，现有指令式 AI 暴露五大痛点。',

    /* ── Summary ── */
    summary:
      '面向跨境商家，构建目标驱动的 AI 智能体体验，把任务协调成本从用户转移给 AI，同时保障商家对关键业务决策的控制权。',
    ...
    background: {
      ...
      why: '随着 Sidekick 能力从对话式问答扩展到上下文理解、任务执行、后台任务与第三方 App 协作，商家可以通过自然语言完成越来越复杂的经营任务。但对于跨应用、长链路、存在失败与风险的复合经营目标，用户仍需要理解 AI 当前掌握的信息、任务状态以及关键操作的影响。',
      targetUsers: '跨境商家——尤其是需要跨多个业务模块协同完成复合经营目标（如 BFCM 大促）的成长型商家。',
      businessGoals:
        '把任务协调成本从用户转移给 AI，同时保障商家对关键业务决策的控制权——让商家从「一步步协调执行细节」转向「定义业务目标、管控关键决策」。',
      constraints: [
        '必须基于 Sidekick AI 现有能力边界与 Shopify 开放生态',
        '支持多语言市场',
        '客户及商家身份需完全匿名化',
      ],
    },
    ...
    challenge: {
      state: 'inferred',
      userProblem:
        '① 复杂目标缺少可干预的任务规划：商家提出高层业务目标后，AI 可以帮助拆解任务，但复杂任务中的依赖关系、并行分支、风险节点与异常状态仍需要更清晰的用户可见结构。② 上下文无法跨任务持续生效：跨页面、跨会话开展工作，必须重复复述业务约束与背景。③ AI 主动参与边界不清：被动等待指令效率低下，过度主动又会带来打扰与失控风险。④ 跨应用任务缺少统一的用户侧状态感知：当一个业务目标需要多个 Shopify / 第三方 App 协同完成时，用户需要在一个任务视角下理解整体进度、依赖关系以及异常节点。⑤ 风险确认机制单一：低风险操作被频繁打断，高风险批量变更缺少充分控制。',
      businessProblem:
        '本该由 AI 承担的任务协调成本，压在了商家身上。',
      designChallenge:
        '不是一味增加自动化，而是让 AI 的能力做到可预测、可干预、失败可恢复。',
    },
    ...
    myRole: {
      responsibilities: [
        ...
        { task: '任务规划', contribution: '独立主导', confidence: 'confirmed', detail: '任务树、任务状态与执行反馈' },
        { task: '风险确认', contribution: '独立主导', confidence: 'confirmed', detail: 'R1–R4 风险分级、影响范围预览' },
        { task: '跨应用工作流', contribution: '独立主导', confidence: 'confirmed', detail: '用户侧任务状态、异常反馈、重试与恢复规则' },
      ],
      note: 'AI 交互模型、意图对齐、上下文交互、任务规划、风险确认与跨应用工作流用户侧体验由我主导设计。协作对象：PM / 工程 / AI-ML 团队 / 用户研究。底层任务运行时与调度代码由工程团队承担，本人负责用户侧任务状态、异常反馈、重试与恢复规则的交互定义。',
    },

    /* ── Design Approach (探索 → 定义 → 原型 → 验证与复盘) ── */
    designApproach: {
      state: 'partial',
      note: '本案例来源于 Shopify 生态 B 端 AI 商业项目；受 NDA 约束，所有界面为示意重绘，Demo 数字仅用于场景演示。关键内容采用三级真实性标注：① 真实项目 ② 事后重整 ③ 概念延展。',
      phases: [
        {
          title: 'Explore · 探索',
    ...
    keyDecisions: [
      {
        id: 1,
        title: 'Where：对话 + 结构化任务界面混合范式（AI 在哪里和用户工作）',
        state: 'confirmed',
        problem:
          '全屏独占界面打断商家现有业务流；全部塞进聊天对话，复杂任务信息堆叠，异常状态不突出。',
        decision:
          '自然语言作为入口，按需唤起任务树、上下文、风险预览、恢复面板。',
        reason:
          '兼顾入门低门槛，同时保证复杂业务任务的可扫描、可干预。',
        alternative: {
          state: 'confirmed',
          text: '原始假设：全部交互可以在聊天对话内完成。替代方案 A（被否决）：全屏独占任务工作区（降级为可选高级视图）。替代方案 B（被否决）：所有交互完全嵌入对话。',
        },
      },
      {
        id: 2,
        title: 'What：分层上下文（AI 知道哪些信息）',
        state: 'confirmed',
        problem:
          'AI 如果记住全部历史对话，会产生上下文污染，旧规则、无关历史干扰当前业务任务。',
        decision:
          '店铺–任务–会话三层分层上下文，区分生命周期，支持用户编辑与覆盖。',
        reason:
          '上下文不应是 AI 黑箱记忆，是用户可查看、可编辑的工作材料。',
        alternative: {
          state: 'confirmed',
          text: '原始假设：AI 持续记住全部历史信息。替代方案（被否决）：AI 完整留存全部历史会话作为记忆。',
        },
      },
      {
        id: 3,
        title: 'How：可展开任务树（AI 如何规划业务）',
        state: 'confirmed',
        problem:
          '复杂任务存在顺序依赖、并行、条件分支、中途失败。简单线性步骤无法表达这些关系，一旦任务偏离预设顺序，用户就会失去对任务结构的理解。',
        decision:
          '可展开任务树，展示任务、子任务、任务依赖、执行状态、异常节点，支持用户展开收起、查看依赖、调整顺序、查看风险、跳过任务、查看失败原因。',
        reason:
          '任务树不是给 AI 自己看的执行结构，而是用户理解和控制 AI 的操作界面。',
        alternative: {
          state: 'confirmed',
          text: '原始假设：AI 直接生成线性步骤即可满足使用。替代方案（被否决）：AI 直接输出纯线性步骤清单。',
        },
      },
      {
        id: 4,
        title: 'When：R1–R4 风险分级确认（AI 什么时候自动、什么时候要人确认）',
        state: 'confirmed',
        problem:
        ...
        decision:
          'R1–R4 分级确认，配套风险预览沙盘。',
        reason:
          '在自动化效率和商家资产安全之间取得可控的平衡点。',
        alternative: {
          state: 'confirmed',
          text: '原始假设：可以用同一套确认逻辑覆盖全部操作。替代方案 A（被否决）：AI 自动执行绝大多数任务。替代方案 B（被否决）：每一步都让用户确认。',
        },
      },
      {
        id: 5,
        title: 'What-if：跨应用任务的统一可见与可控（出错之后怎么办）',
        state: 'confirmed',
        problem:
          '真实商业任务往往需要多个应用共同完成。如果每个应用独立运行，状态分散，部分失败缺少统一隔离、重试、回滚。',
        decision:
          '状态聚合：提供全局任务视野，让用户能够看到跨应用任务整体进度；恢复规则：定义体验层异常隔离、重试与恢复的操作入口。',
        reason:
          '让多应用协同，在用户视角表现为一个完整、既看得见也可操作的业务任务。',
        alternative: {
          state: 'confirmed',
          text: '原始假设：依靠各个第三方应用自身，即可完成跨应用工作流。替代方案（被否决）：完全依赖各个第三方应用自己处理任务状态与异常逻辑。备注：仅定义用户侧交互约束，底层系统运行时代码实现归属工程团队。',
        },
      },
    ],
    decisionsNote: null,
    ...
    outcome: {
      ...
      qualitative: [
        ...
        '沉淀一套可复用的 B 端智能体体验机制：分层上下文、任务树规划、R1–R4 风险体系、跨应用任务统一可见与可控机制',
    ...
    reflection: {
      ...
      note:
        '项目复盘：我把「用户下达指令，AI 执行」重新定义为「用户提出业务目标，AI 理解、规划、执行，并在关键节点交还用户控制权」。AI-UX 设计的重点，不只是追求大模型能力更强；而是把模型能力转化为用户能够理解、控制、出问题可以恢复的产品体验。企业级 AI 的信任，来源于可预测，而不是完全自动化。下一阶段探索：智能体自主程度如何动态适配不同商家运营习惯、长期业务上下文如何控制数据保留范围、跨应用失败如何定义一致的回滚边界、不同规模商家的风险策略如何配置。',
    },
  },
};
